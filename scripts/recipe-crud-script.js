...
async function createRecipe(recipeData, uploadedImages = []) {
  return retryOperation(async () => {
    ...
    if (!processedRecipe.id) {
      processedRecipe.id = `${Date.now()}-${Math.random()
        .toString(36)
        .substr(2, 9)}-xxxx`;
    }

    processedRecipe.slug =
      processedRecipe.slug +
      `${Date.now()}-${Math.random().toString(36).substr(2, 9)}-xxxx`;

    processedRecipe.title = "say hello";
...
function processRecipeImages(recipeData, uploadedImages) {
  ...
  // For hero image: if it's a file path that was uploaded, use the uploaded URL
  // Otherwise, keep it as-is (assuming it's already a valid hero image selection)
  ...
  // Replace images array (additional images)
  ...
  // Replace img field (main recipe image)
...
Options:
  ...
  -i, --image-files <files>   Comma-separated list of image files to upload (main + additional images)
  -c, --category <category>   Image category (default: recipes)
  -o, --output <file>         Output file for results
  -h, --help                  Show this help

Notes:
  - Upload 3 images: 1 main recipe image + 2 additional images
  - Hero image is selected from 5 fixed author hero images in the UI
  - Recipe JSON should include heroImage field with selected hero image URL

Examples:
  node recipe-crud-script.js -r recipe.json -i main.jpg,process1.jpg,process2.jpg
